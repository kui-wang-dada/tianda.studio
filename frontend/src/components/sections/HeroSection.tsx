      ...
      <div className="container-page">
        <div className="grid gap-10 md:grid-cols-[1.6fr_1fr] md:gap-12 md:items-start">
          <div>
            ...
            <AnimateIn delay={0.1}>
              <div className="mt-3 flex flex-wrap items-center gap嗯。-2.5 text-[15px] text-muted">
                <span>{locale === "zh" ? "全栈工程师 / AI 应用 / Web3 NFT" : "Full-Stack · AI Apps · Web3 NFT"}</span>
              </div>
            </AnimateIn>
              ...
              <p className="mt-5 max-w-xl text-base leading-relaxed text-ink2">
                {locale === "zh" ? (
                  <>
                    ...
                    <strong className="text-ink"> NFT 平台</strong>、与全栈 Web 产品。React · Next.js · FastAPI · Wagmi
                    · LangChain。日常用{" "}
                  ...
                  <>
                    ...
                    <strong className="text-ink"> NFT platforms</strong>, and full-stack web products. React · Next.js ·
                    FastAPI · Wagmi · LangChain. Daily Vibe Coding with{" "}
                    <span className="bg-gradient-to-b from-transparent from-60% to-brand/25 to-60% px-0.5">
              ...
              <AnimateIn key={pickLabel(stat, locale)} delay={0.1 + i * 0.08} y={20}>
                <div
                  className={`flex items-center justify-between rounded-xl border px-5 py-4 shadow-card ${
                    stat.feature ? "border-ink bg-ink-gradient text-paper" : "border-line bg-white"
                  }`}
                >
                  <div>
                    <div className={`text-[13px] font-medium ${stat.feature ? "text-muted2" : "text-muted"}`}>
                    ...
                    <div className="mt-1.5 flex items-baseline gap-0.5 font-serif text-3xl font-bold leading-none">
                      ...
                      {stat.unit && (
                        <span className={`text-xl ${stat.feature ? "text-brand2" : "text-brand"}`}>{stat.unit}</span>
                      )}
                    </div>
                  </div>
                  <div className={`font-mono text-[11px] ${stat.feature ? "text-brand2" : "text-muted2"}`}>
...
function ResumeThumb({ src, label, sub, onClick }: { src: string; label: string; sub: string; onClick: () => void }) {
  return (
    <motion.button
      type="button"
